...
Hypergraft is a bounded HTML-over-HTTP and WebSocket protocol with a JavaScript-required browser runtime. Version 1 of the Rust crate targets Axum and owned HTML templates.

## Notes to agent

- You may run `mise` commands. At the end of your work, if you have changed any code, you MUST run `mise run clean`. If there are errors or warnings you MUST fix them, then run `mise run clean` again.
...
- The version 1 Rust host is Axum and owned HTML templates. Do not add a second host adapter unless a plan asks for it.
